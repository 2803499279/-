...
- (void)setupTitleArr {
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        for (NSUInteger i = 0; i < _title_Arr.count; i++) {
            ...
            _title_btn.titleLabel.font = FONT(18);
            _title_btn.tag = i;
            
            // 计算内容的Size
            CGSize buttonSize = [self sizeWithText:_title_Arr[i] font:FONT(18) maxSize:CGSizeMake(MAXFLOAT, button_H)];
        ...
        _indicatorView.SG_y = button_H-10*YZAdapter;
        [self addSubview:_indicatorView];
        
        // 指示器默认在第一个选中位置
        // 计算TitleLabel内容的Size
        CGSize buttonSize = [self sizeWithText:firstButton.titleLabel.text font:FONT(18) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
...
- (void)setupImageAndTitleArr {
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        for (NSUInteger i = 0; i < _title_Arr.count; i++) {
            ...
            _image_title_btn.titleLabel.font = FONT(18);
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        if (self.segmentedControlIndicatorType == SGSegmentedControlIndicatorTypeCenter) {
            ...
            [UIView animateWithDuration:indicatorViewTimeOfAnimation animations:^{
                // 计算内容的Size
                CGSize buttonSize = [self sizeWithText:button.titleLabel.text font:FONT(18) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height - indicatorViewHeight)];
