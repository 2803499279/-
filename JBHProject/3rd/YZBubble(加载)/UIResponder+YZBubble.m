...
#import "UIResponder+YZBubble.h"

@implementation UIResponder (YZBubble)

- (YZBubbleInfo *)getDefaultRightBubbleInfo{
    YZBubbleInfo *info = [[YZBubbleInfo alloc] init];
...
- (void)showRightWithTitle: (NSString *)title autoCloseTime: (CGFloat)autoCloseTime{
    YZBubbleInfo *info = [self getDefaultRightBubbleInfo];
    info.title = title;
    [[YZBubbleView defaultBubbleView] showWithInfo: info autoCloseTime: autoCloseTime];
}

- (YZBubbleInfo *)getDefaultRoundProgressBubbleInfo{
    YZBubbleInfo *info = [[YZBubbleInfo alloc] init];
    info.iconAnimation = ^(CAShapeLayer *layer){
        ...
        CAShapeLayer *alphaLineLayer = [CAShapeLayer layer];
        alphaLineLayer.path = circlePath.CGPath;// 设置透明圆形的绘图路径
        alphaLineLayer.strokeColor = YZEssentialColor.CGColor;// 设置图层的透明圆形的颜色
        alphaLineLayer.lineWidth = STROKE_WIDTH;// 设置圆形的线宽
        alphaLineLayer.fillColor = [UIColor clearColor].CGColor;// 填充颜色透明
    ...
    info.backgroundColor = [UIColor colorWithRed: 1 green:1 blue:1 alpha:0.8];
    return info;
}

- (void)showRoundProgressWithTitle: (NSString *)title{
    
    YZBubbleInfo *info = [self getDefaultRoundProgressBubbleInfo];
    info.title = title;
    [[YZBubbleView defaultBubbleView] showWithInfo: info];
}



- (YZBubbleInfo *)getDefaultErrorBubbleInfo{
    YZBubbleInfo *info = [[YZBubbleInfo alloc] init];
    info.iconAnimation = ^(CAShapeLayer *layer){
        const int STROKE_WIDTH = 3;// 默认的划线线条宽度
...
- (void)showErrorWithTitle: (NSString *)title autoCloseTime: (CGFloat)autoCloseTime{
    YZBubbleInfo *info = [self getDefaultErrorBubbleInfo];
    info.title = title;
    [[YZBubbleView defaultBubbleView] showWithInfo: info autoCloseTime: autoCloseTime];
}
...
- (void)hideBubbleAfter:(CGFloat)duration {
    [[YZBubbleView defaultBubbleView] hideWithCloseTime:duration];
}

- (void)showBubbleWithInfo: (YZBubbleInfo *)info{
    [[YZBubbleView defaultBubbleView] showWithInfo: info];
}

- (void)showBubbleWithInfo: (YZBubbleInfo *)info time: (CGFloat)time{
    [[YZBubbleView defaultBubbleView] showWithInfo: info autoCloseTime: time];
}
