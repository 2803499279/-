...
@interface YZBubbleView()

@property (nonatomic, strong) UIImageView *iconImageView;
@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, assign) BOOL *isShowing;// 是否正在显示中
@property (nonatomic, strong) NSMutableDictionary<NSString*, YZBubbleInfo*> *infoDic;
/// @brief 当前正在显示的泡泡信息对象
@property (nonatomic, strong) YZBubbleInfo *currentInfo;
/// @brief 当前自定义动画绘图的图层
...
@implementation YZBubbleView

+ (YZBubbleView *)defaultBubbleView {
    static YZBubbleView *defaultBubbleView = nil;
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        if (defaultBubbleView == nil) {
            defaultBubbleView = [[YZBubbleView alloc] init];
        }
    });
...
- (void)showWithInfo: (YZBubbleInfo *)info{
         ...
         usingSpringWithDamping: 0.8
          initialSpringVelocity:0.9
                     ...
                     animations:^{
                        ...
                        if (info.iconArray.count == 1){// 显示单张图片
                             [_currentTimer invalidate];
    ...
    [UIView animateWithDuration: 0.45
                     ...
                     animations:^{
                         _titleLabel.textColor = MainFont_Color;
    ...
    if ([info.title isEqualToString:@"加载中..."]) {
        [self requestTaskSt];
    }
}
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:{
                // 未知网络状态
                // 添加网络判断
                // 添加定时器, 如果时间超过6秒取消加载提示
                self.timer = [NSTimer timerWithTimeInterval:6 target:self selector:@selector(requestTaskState) userInfo:nil repeats:YES];
