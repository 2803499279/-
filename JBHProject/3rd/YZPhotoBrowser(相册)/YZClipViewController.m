...
@interface YZClipViewController ()

@end

@implementation YZClipViewController
...
-(void)CreatUI
{
    ...
    double height = (_image.size.height / _image.size.width) * self.view.frame.size.width;
    ...
    UIView *clipView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 50, self.view.frame.size.width, 50)];
    clipView.backgroundColor = BackGround_Color;
    [self.view addSubview:clipView];
    
    UIButton * clipBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [clipBtn  setExclusiveTouch :YES];
    [clipBtn setTitle:@"选择" forState:UIControlStateNormal];
    [clipBtn setTitleColor:MainFont_Color forState:UIControlStateNormal];
    ...
    [clipBtn setFrame:CGRectMake(Screen_W-80, self.view.frame.size.height - 50, 80, 50)];
...
//    CGPoint center = self.view.center;
    ...
    self.circularFrame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
    ...
    {
        [path appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(0, 0, ScreenWidth, ScreenHeight)]];
    }
...
-(void)MakeImageViewFrameAdaptClipFrame
{
    ...
    if(height < self.circularFrame.size.height)
    {
        ...
        height = (height / width) * self.circularFrame.size.height;
...
//    CGFloat rationScale = (width /_image.size.width);
    ...
    CGFloat origX = self.circularFrame.origin.x;
    CGFloat origY = self.circularFrame.origin.y;
    CGFloat oriWidth = _image.size.width;
    CGFloat oriHeight = _image.size.height;
