...
- (void)createToolbar
{
    ...
    _BlackImageBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_BlackImageBtn  setExclusiveTouch :YES];
    _BlackImageBtn.frame = CGRectMake(40*YZAdapter, 40*YZAdapter, 30*YZAdapter, 30*YZAdapter);
    _BlackImageBtn.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    [_BlackImageBtn setImage:[UIImage imageNamed:@"CamerBC.png"] forState:UIControlStateNormal];
    [_BlackImageBtn addTarget:self action:@selector(BlackImage:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_BlackImageBtn];
    
    
    _selectImageBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_selectImageBtn  setExclusiveTouch :YES];
    _selectImageBtn.frame = CGRectMake(Screen_W-70*YZAdapter, 40*YZAdapter, 30*YZAdapter, 30*YZAdapter);
    _selectImageBtn.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    [_selectImageBtn setImage:[UIImage imageNamed:@"Camerdelete.png"] forState:UIControlStateNormal];
...
- (void)BlackImage:(UIButton *)sender {
    if ([self.photoBrowserdelegate respondsToSelector:@selector(deletedPictures:Str:)]) {
        [self.photoBrowserdelegate deletedPictures:self.selectImageSet Str:@"清除"];
...
-(void)selectImage:(UIButton *)sender
{
    if (_is_selectImageBtnBool == YES) {
        ...
        DLog(@"第%d张图片",_toolbar.photosIndex);
        ...
        if ([self.photoBrowserdelegate respondsToSelector:@selector(deletedPictures:Str:)]) {
            [self.photoBrowserdelegate deletedPictures:self.selectImageSet  Str:@"不清除"];
        }
    ...
    }else
    {
        ...
        _is_selectImageBtnBool = YES;
        [sender setImage:[UIImage imageNamed:@"Camerdelete.png"] forState:UIControlStateNormal];
    }
...
//    DLog(@"公%@张图片",self.selectImageSet);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    DLog(@"selectArray----- %@",self.selectImageSet);
}
