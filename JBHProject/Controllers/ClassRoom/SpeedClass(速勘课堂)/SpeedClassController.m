//
//  SpeedClassController.m
//  JBHProject
//
//  Created by zyz on 2017/6/28.
//  Copyright © 2017年 聚宝汇. All rights reserved.
//

#import "SpeedClassController.h"
#import "YZViewJavaScriptBridge.h"
#import "NJKWebViewProgress.h"
#import "NJKWebViewProgressView.h"

extern NSString *NumName = @"";


@interface SpeedClassController () <UIWebViewDelegate, ViewJavaScriptBridgeDelegate, NJKWebViewProgressDelegate>
{
...
@implementation SpeedClassController
- (void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
    
    if ([NumName isEqualToString:@"iOS"]) {
...
- (void) webviewRequest {
    ...
    [self customNavigationBarTitle:@"速勘课堂" backGroundImageName:nil];
    
    self.webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, Screen_W, Screen_H-64)];
    
    NSString *URLStr = [NSString stringWithFormat:@"%@", ARTICLE];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    if ([self.webView canGoBack])
    {
        if (![title isEqualToString:@"速勘课堂"])
        {
        [self customPushViewControllerNavBarTitle:title backGroundImageName:nil];
        self.tabBarController.tabBar.hidden = YES;
    ...
    {
        [self hiddenLeftButton];
        self.webView.frame = CGRectMake(0, 0, Screen_W, Screen_H-64-49);
        self.tabBarController.tabBar.hidden = NO;
        if (title.length == 0) {
            self.naviTitle = @"速勘课堂";
        }else if([title isEqualToString:@"速勘课堂"]){
            self.naviTitle = @"速勘课堂";
        }
    }
}
...
-(void)webViewProgress:(NJKWebViewProgress *)webViewProgress updateProgress:(float)progress {
    ...
    //    if (progress == 1.0) {
    //        _label = [[UILabel alloc] initWithFrame:CGRectMake(0, Screen_H/2-64, Screen_W, 40*YZAdapter)];
    //        _label.text = @"请求失败, 请检查您的网络连接";
    //        _label.textAlignment = NSTextAlignmentCenter;
    //        _label.textColor = TimeFont_Color;
    //        _label.font = FONT(16);
    //        [self.webView addSubview:_label];
    //    }
}
